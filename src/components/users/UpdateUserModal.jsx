...
const UpdateUserModal = ({ open, handleClose, id, onSuccess }) => {
        ...
        if (open && id) {
            const fetchUser = async () => {
                try {
                    ...
                    const user = res.data.find((entry) => entry._id === id);
                    if (!user) {
                        return;
                    }
                    setForm({
    ...
    return (
        <Dialog
            ...
            <DialogContent sx={{ p: 3, display: "flex", flexDirection: "column", gap: 2.5, mt: 1 }}>
                ...
                <TextField margin="dense" id="password" name="password" label="Password" type="password" fullWidth variant="outlined" sx={textFieldStyles} value={form.password} onChange={handleFormChange} helperText="Leave blank to keep the current password unchanged." />
            </DialogContent>
